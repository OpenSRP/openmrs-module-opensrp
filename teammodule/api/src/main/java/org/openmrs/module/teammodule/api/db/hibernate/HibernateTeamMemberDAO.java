...
public class HibernateTeamMemberDAO implements TeamMemberDAO {
	...
	return sessionFactory.getCurrentSession().createSQLQuery("select count(*) from team_member where voided = 0 and team_id ="+teamId);
	}*/
	...
	public List<TeamMember> searchMember(String name) {
		 ...
		 */
		return sessionFactory.getCurrentSession().createQuery("from TeamMember tm join tm.person p join p.names pn where pn.givenName like :name or pn.familyName like :name or tm.identifier like :name").setString("name", "%" + name + "%")
	...
	public List<TeamMember> searchMemberByTeam(String name,int teamId) {
		...
		return sessionFactory.getCurrentSession().createQuery("from TeamMember tm join tm.person p join p.names pn join tm.team t where pn.givenName like :name or pn.familyName like :name or tm.identifier like :name and t.teamId = :teamId").setString("name", "%" + name + "%").setInteger("teamId", teamId)
				.setResultTransformer(new ResultTransformer() {
					@SuppressWarnings("rawtypes")
					final Map<Integer, Date> tmMap = new LinkedHashMap();
